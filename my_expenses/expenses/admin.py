...
#first action
#allows us to multicheckbox our instance and change staus is_paid wiht one click on admin page
#when we check the instance it creates a queryset
...
@admin.register(PaymentMethod) # register a model on admin page using this command
...
class BillAdmin(admin.ModelAdmin):
    save_as = True # allows us to create same instance with different id (copy paste method)
    save_on_top = True
    list_display = ['date_expired', 'category', 'tag_final_value','payment_method', 'is_paid'] # define what field/functions will show on main table
    list_filter = ['category', 'is_paid', 'date_expired', 'payment_method']# defines filters
    search_fields = ['title', 'category__title'] #creat search bar and here we defines where t0 search and final on actions we register which actions we want to use
